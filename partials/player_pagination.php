...
$champion = isset($_GET["champion"]) ? trim($_GET["champion"]) : "";
...
if ($search !== "") {
    $where[] = "riot_username LIKE :search";
...
if ($sort === "nom-asc") {
    $order_sql = "ORDER BY riot_username ASC";
} elseif ($sort === "nom-desc") {
    $order_sql = "ORDER BY riot_username DESC";
} elseif ($sort === "rang") {
    $order_sql = "ORDER BY FIELD(rank, 'iron','bronze','silver','gold','platinum','emerald','diamond','master','grandmaster','challenger')";
}


$sql_count = "SELECT COUNT(id) as total FROM player_posts $where_sql";
...
$sql = "SELECT * FROM player_posts $where_sql $order_sql LIMIT :start_from, :results_per_page";
...
function build_filters_querystring($extra = []) {
    $params = [
        "search"   => $_GET["search"]   ?? "",
        "role"     => $_GET["role"]     ?? "",
        "rank"     => $_GET["rank"]     ?? "",
